...
function preload() {
  ...
  for (let i = 1; i <= 40; i++) {
    if(i.toString().length == 1){
      photos.push(loadImage(`/vc/sketches/shaders/paintings/00000${i}.jpg`));
    }else{
      photos.push(loadImage(`/vc/sketches/shaders/paintings/0000${i}.jpg`));
    }
  }
}
...
function setup() {
  ...
  resolution = createSlider(10, 200, sclDiv, 1);
...
function getIndex(r, g, b){
  ...
  for(let j = 10; j <= 100; j += 10){
    for(let i = 0 ; i < photos.length && complete == false; i++){
      if((reds[i]> (r - j) && reds[i] < (r + j)) && (greens[i]> (g - j) && greens[i] < (g + j)) && (blues[i]> (b - j) && blues[i] < (b + j))){
        selected = i;
        complete = true;
        break;
      }
...
function draw() {
  ...
  if(!video_on.checked()){
    ...
    smaller.copy(image_src, 0, 0, image_src.width, image_src.height, 0, 0, w, h);
    smaller.loadPixels();
    for (let x = 0; x < w; x++) {
      for (let y = 0; y < h; y++) {
        let index = x + y * w;
    ...
    background(255);
    smaller = createImage(w, h);
    smaller.copy(video_src, 0, 0, video_src.width, video_src.height, 0, 0, w, h);
    smaller.loadPixels();
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        let index = x + y * w;
